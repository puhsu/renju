...
def check_horizontal(board, pos):
    ...
    length += sequence_length(
        board,
        itertools.repeat(i),
        range(j + 1, min(j + gomoku.rules.Game.line_length, gomoku.rules.Game.width)),
        player
    )
    ...
    length += sequence_length(
        board,
        itertools.repeat(i),
        range(j - 1, max(j - gomoku.rules.Game.line_length, -1), -1),
        player
    )

    return length >= gomoku.rules.Game.line_length
...
def check_vertical(board, pos):
    ...
    length += sequence_length(
        board,
        range(i + 1, min(i + gomoku.rules.Game.line_length, gomoku.rules.Game.height)),
        itertools.repeat(j),
        player
    )
    ...
    length += sequence_length(
        board,
        range(i - 1, max(i - gomoku.rules.Game.line_length, -1), -1),
        itertools.repeat(j),
        player
    )
